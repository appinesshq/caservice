...
import (
	...
	uc "github.com/appinesshq/caservice/business/user/usecases"
	...
	validate "github.com/appinesshq/caservice/foundation/validation"
...
func Test_Users(t *testing.T) {
	...
	tests := UserTests{
		app: handlers.APIMux(handlers.APIMuxConfig{
			...
			Log:          test.Log,
			Auth:         test.Auth,
			Repositories: &data.Repositories{UserRepo: pg.NewStore(test.Log, test.DB)},
		}),
...
func (ut *UserTests) getToken200(t *testing.T) {
	...
	t.Log("Given the need to issues tokens to known users.")
	{
		...
		t.Logf("\tTest %d:\tWhen fetching a token with valid credentials.", testID)
		{
			...
			// TODO Should we ensure the token is valid?
		}
	}
}
...
func (ut *UserTests) postUser400(t *testing.T) {
	body, err := json.Marshal(&uc.NewUser{})
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Log("Given the need to validate a new user can't be created with an invalid document.")
	{
		...
		t.Logf("\tTest %d:\tWhen using an incomplete user value.", testID)
		{
			...
			fields := validate.ValidationErrors{
				{Field: "name", Err: "name is a required field"},
				{Field: "email", Err: "email is a required field"},
			...
			sorter := cmpopts.SortSlices(func(a, b validate.FieldValidationError) bool {
